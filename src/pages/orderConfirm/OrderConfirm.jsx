...
const OrderConfirm = () => {
  const { cartItems, food_list, getCartAmount, clearCart } = useContext(contextParse);
  ...
  const [message, setMessage] = useState({ text: "", type: "" }); // For success/failure message visibility
  ...
  const handleOrderSubmit = async (e) => {
    ...
    const selectedItems = food_list.filter(item => cartItems[item._id] > 0).map(item => ({
      itemId: item._id,
      name: item.name,
      price: item.price,
    ...
    const orderData = {
      ...
      items: selectedItems,
      total: getCartAmount() + 2, // include delivery
    };
